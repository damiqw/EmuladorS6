...
void HackServerProtocolCore(int index,BYTE head,BYTE* lpMsg,int size) // OK
{
	...
	if (gServerInfo.m_WriteProtocolLog)
		LogAdd(LOG_RED, "header: 0x%x, ip_addr: %s",head,gClientManager[index].m_IpAddr);
	...
	switch(head)
	{
		...
		case 0x03:
			...
			break;
		case 0x16:
			CGAccountInfoRecv((SDHP_CLIENT_ACCOUNT_INFO_RECV*)lpMsg, index);
			break;
		default:
			LogAdd(LOG_RED,"Wrong Packet");
...
void CGAccountInfoRecv(SDHP_CLIENT_ACCOUNT_INFO_RECV* lpMsg, int index)
{
	gClientManager[index].SetAccountInfo(lpMsg->account, lpMsg->name);
	gHelperPlayers.set_account_info(index);
}
...
void HCClientRebootSend(int index, int type)
{
	SDHP_CLIENT_DISCONNECT_SEND pMsg;
...
void GCClientScreenShotSend(int index)
{
	SDHP_CLIENT_DISCONNECT_SEND pMsg;
...
void HCGetAccountDataSend(int index)
{
	SDHP_CLIENT_DISCONNECT_SEND pMsg;
