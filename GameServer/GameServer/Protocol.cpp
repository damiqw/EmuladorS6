...
void ProtocolCore(BYTE head,BYTE* lpMsg,int size,int aIndex,int encrypt,int serial) // OK
{
	...
	switch(head)
	{
...
#if(HAISLOTRING)
				case 0x09:
					gPersonalShop.CGPEquipShopSetItemPriceRecv((PMSG_PSHOP_SET_ITEM_PRICE_RECV*)lpMsg, aIndex);
					break;
#endif
			}
			break;
...
#if(HAISLOTRING)
				case 0x0A:
					g_pNewMyInventory.CGItemSellRecv((PMSG_EQUIP_ITEM_SELL_RECV*)lpMsg, aIndex); //ZEUS UP20
					break;
#endif
			}
			break;
		...
		#if(NEW_POSTITEM)
		case 0x78:
			GCPostItemProc(lpMsg, size, aIndex); //post item
			break;
		#endif
#if(RIGHT_CLICK)
		case 0x79:
			gItemManager.CGMoveItemProc((PMSG_MOVEITEM*)lpMsg, aIndex); //right click
			break;
#endif
		case 0x81:
		...
		case 0xBF:
			switch(lpMsg[3])
			{
				...
				case 0x0E:
					...
					break;
				case 0xFE:
					JCSwitchServer.SwitchServerReq((JCCG_MOVESERVER_REQ*)lpMsg, aIndex);
					break;
				case 0x17:
					#if(GAMESERVER_UPDATE>=501)
		...
		case 0xF3:
			switch(lpMsg[3])
			{
				...
				case 0x31:
					...
					break;
#if(AUTO_MOVE)
				case 0x37: //Info Auto Move Running
					GCAutoMove((AUTOMOVE_REQ*)lpMsg, aIndex);
					break;
#endif
				case 0x52:
					#if(GAMESERVER_UPDATE>=401)
				...
				case 0xF6:
					...
					break;
				case 0xF7:
					gWarehouse.WarehouseChangeRecv((PMSG_WAREHOUSE_CHANGE_RECV*)lpMsg, aIndex);
					break;
#if(FLAG)
				case 0x60:
					gCustomBuyVip.BuyFlag(aIndex, (BUYFLAG_REQ*)lpMsg);
					break;
				case 0x61:
					gCustomBuyVip.CancelFlag(aIndex, (CANCELFLAG_REQ*)lpMsg);
					break;
#endif
			}
			break;
		...
		case 0xD3:
			...
			case 0x4C:
				gInvasionManager.SendMonter(aIndex, (EVENT_INVASION_ACTION*)lpMsg);
				break;
#endif
#if(CUSTOM_BRANKINGNEW)
			case 0x40: //GetInfoCharTop
			{
				gCustomRanking.CGetInfoCharTop((REQUESTINFO_CHARTOP*)lpMsg, aIndex);
			}
			break;
#endif
#if(LuckyWheelV2)
			case 0x8A: //Nhan Thuong
			{
				XULY_CGPACKET_SOLAN* mlpMsg = (XULY_CGPACKET_SOLAN*)lpMsg;
				if (!mlpMsg->ThaoTac) { return; }
				//LogAdd(LOG_RED, "0x4A Nhan Thuong %d", mlpMsg->ThaoTac);
				gCustomVongQuay.ActionVongQuay(aIndex, mlpMsg->ThaoTac, mlpMsg->SoLan);
			}
			break;
			case 0x8B: //Get List Nhan Thuong
			{
				XULY_CGPACKET* mlpMsg = (XULY_CGPACKET*)lpMsg;
				if (!mlpMsg->ThaoTac) { return; }
				//LogAdd(LOG_RED, "0x4B Get List %d", mlpMsg->ThaoTac);
				gCustomVongQuay.SendListNhanThuong(aIndex, mlpMsg->ThaoTac);
			}
			break;
			case 0x8C: //OpenWIndow
			{
				XULY_CGPACKET* mlpMsg = (XULY_CGPACKET*)lpMsg;
				if (!mlpMsg->ThaoTac) { return; }
				//LogAdd(LOG_RED, " OpenWIndowVopngquay, 0x8C Get List %d", mlpMsg->ThaoTac);
				gCustomVongQuay.UserSendClientInfo(lpObj->Index);
			}
			break;
#endif
			}
		...
		case 0xD5:
			...
			{
			case 0x11:
				gNewCashShop.GsSendPackSize11(lpMsg, aIndex);
				break;
			case 0x12:
				gNewCashShop.GsSendPackSize12(lpMsg, aIndex);
				break;
			case 0x13:
				gNewCashShop.GsSendPackSize13(lpMsg, aIndex);
				break;
			}
			break;
		case 0xFF:
...
void CGEventRemainTimeRecv(PMSG_EVENT_REMAIN_TIME_RECV* lpMsg,int aIndex) // OK
{
	...
	switch(lpMsg->EventType)
	{
		case 1:
		{
			...
			{
				GCNewMessageSend(aIndex,0,0,"%s",gMessage.GetMessage(149));
				return;
			}
		}break;
		case 2:
		...
		case 4:
		{
			...
			{
				GCNewMessageSend(aIndex,0,0,"%s",gMessage.GetMessage(59));
				return;
			}
		}break;
		case 5:
		{
			...
			{
				GCNewMessageSend(aIndex,0,0,"%s",gMessage.GetMessage(181));
				return;
			}
		}break;
	}
...
#if(FLAG)
			info.bHaeManyFlag = lpTarget->Flag;
#endif
		}
		else
		{
...
#if(NEW_POSTITEM)
void ItemByteConverter(BYTE* lpMsg, CItem item) // OK
{
	...
	memcpy(&lpMsg[7], &item.m_SocketOption[0], MAX_SOCKET_OPTION);
}

//post item
void GCPostItemProc(BYTE* lpMsg, int size, int aIndex) //ok  
{
	if(gServerInfo.m_CommandPostItem == 0)
	{
		return;

	}

	if (size != sizeof(PMSG_POST_ITEM_RECV)) {
		return;
	}
	PMSG_POST_ITEM_RECV* Data = (PMSG_POST_ITEM_RECV*)lpMsg;
	...
	if (Data->pos < 0 || Data->pos > 128)
	{
		return;
	}

	if (!lpObj->Inventory[Data->pos].IsItem())
	{
		return;
	}

	if(gServerInfo.m_CommandPostItemEnable[lpObj->AccountLevel] == 0)
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(940));
		return;
	}

	if(lpObj->Level < gServerInfo.m_CommandPostItemLevel[lpObj->AccountLevel])
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(941),gServerInfo.m_CommandPostItemLevel[lpObj->AccountLevel]);
		return;
	}

	if(lpObj->Reset < gServerInfo.m_CommandPostItemReset[lpObj->AccountLevel])
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(942),gServerInfo.m_CommandPostItemReset[lpObj->AccountLevel]);
		return;
	}

	if(lpObj->Money < ((DWORD)gServerInfo.m_CommandPostItemMoney[lpObj->AccountLevel]))
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(943),gServerInfo.m_CommandPostItemMoney[lpObj->AccountLevel]);
		return;
	}

	if(lpObj->Coin1 < ((DWORD)gServerInfo.m_CommandPostItemCoin1[lpObj->AccountLevel]))
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(944),gServerInfo.m_CommandPostItemCoin1[lpObj->AccountLevel]);
		return;
	}

	if(lpObj->Coin2 < ((DWORD)gServerInfo.m_CommandPostItemCoin2[lpObj->AccountLevel]))
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(945),gServerInfo.m_CommandPostItemCoin2[lpObj->AccountLevel]);
		return;
	}

	if(lpObj->Coin3 < ((DWORD)gServerInfo.m_CommandPostItemCoin3[lpObj->AccountLevel]))
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(946),gServerInfo.m_CommandPostItemCoin3[lpObj->AccountLevel]);
		return;
	}

	if (lpObj->Ruud < ((DWORD)gServerInfo.m_CommandPostItemRuud[lpObj->AccountLevel]))
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(946), gServerInfo.m_CommandPostItemRuud[lpObj->AccountLevel]);
		return;
	}

	DWORD tick = (GetTickCount()-lpObj->PostTime)/1000;

	if(tick < ((DWORD)gServerInfo.m_CommandPostItemDelay[lpObj->AccountLevel]))
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(68),(gServerInfo.m_CommandPostItemDelay[lpObj->AccountLevel]-tick));
		return;
	}
	
	lpObj->PostTime = GetTickCount();

	lpObj->Money -= gServerInfo.m_CommandPostItemMoney[lpObj->AccountLevel];

	GCMoneySend(lpObj->Index,lpObj->Money);

	GDSetCoinSend(lpObj->Index,-(gServerInfo.m_CommandPostItemCoin1[lpObj->AccountLevel]),-(gServerInfo.m_CommandPostItemCoin2[lpObj->AccountLevel]),-(gServerInfo.m_CommandPostItemCoin3[lpObj->AccountLevel]), -(gServerInfo.m_CommandPostItemRuud[lpObj->AccountLevel]),"ItemPost");

	SDHP_POST_ITEM_SEND pMsg = { 0, };
	pMsg.header.set(0x78, sizeof(pMsg));
	pMsg.MapServerGroup = gMapServerManager.GetMapServerGroup();
	memcpy(pMsg.chatid, lpObj->Name, sizeof(pMsg.chatid)-1);
	pMsg.Type = lpObj->Inventory[Data->pos].m_Index;
	pMsg.Dur = lpObj->Inventory[Data->pos].m_Durability;
	ItemByteConverter(pMsg.Item, lpObj->Inventory[Data->pos]);
	//	LogAdd(LOG_RED,"AA");
	pMsg.PeriodTime = lpObj->Inventory[Data->pos].m_IsPeriodicItem ? lpObj->Inventory[Data->pos].m_PeriodicItemTime : 0;
	memcpy(pMsg.pre, Data->pre, sizeof(pMsg.pre) - 1);
	memcpy(pMsg.suf, Data->suf, sizeof(pMsg.suf) - 1);
...
void CCustomBuyVip::BuyFlag(int aIndex, BUYFLAG_REQ* lpMsg)
{
	LPOBJ lpObj = &gObj[aIndex];
	...
	lpObj->Flag = lpMsg->Flag;

	lpObj->CloseCount = 1;
	lpObj->CloseType = 1;
}

void CCustomBuyVip::CancelFlag(int aIndex, CANCELFLAG_REQ* lpMsg)
{
	LPOBJ lpObj = &gObj[aIndex];
