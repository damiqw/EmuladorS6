...
void DataServerProtocolCore(int index,BYTE head,BYTE* lpMsg,int size) // OK
{
	...
	switch(head)
	{
...
#if(HAISLOTRING)
				case 0x01:
					g_pNewMyInventory.GDPetInventoryRecv((SDHP_PET_INVENTORY_RECV*)lpMsg, index);
					break;
				case 0x31:
					g_pNewMyInventory.GDPetInventorySaveRecv((SDHP_PET_INVENTORY_SAVE_RECV*)lpMsg);
					break;
#endif
			}
			break;
		...
		#if(NEW_POSTITEM)
		case 0x78:
			GDPostItemProc(lpMsg, size, index);
			break;
		#endif
		case 0x80:
			switch(((lpMsg[0]==0xC1)?lpMsg[3]:lpMsg[4]))
		...
		case 0xD3:
			switch (((lpMsg[0] == 0xC1) ? lpMsg[3] : lpMsg[4]))
			{
				...
				case 0x40:
				{
					CGetInfoCharTop((REQUESTINFO_CHARTOP*)lpMsg, index);
				}
			break;
#endif
			}
...
void GDCharacterInfoRecv(SDHP_CHARACTER_INFO_RECV* lpMsg,int index) // OK
{
	...
	else
	{
		...
		pMsg.CheckFlag = gQueryManager.GetAsInteger("CheckFlag");
		...
		gQueryManager.ExecQuery("SELECT ServerName FROM MEMB_STAT WHERE memb___id='%s'",lpMsg->account);

		gQueryManager.Fetch();
		
		gQueryManager.GetAsString("ServerName",pMsg.ServerName,sizeof(pMsg.ServerName));
...
void GDCharacterInfoSaveRecv(SDHP_CHARACTER_INFO_SAVE_RECV* lpMsg) // OK
{
	...
	gQueryManager.ExecQuery("UPDATE Character SET Flag=%d,CheckFlag=%d WHERE Name='%s'", lpMsg->Flag, lpMsg->CheckFlag, lpMsg->name);
	gQueryManager.Close();
}
...
{
	gQueryManager.ExecQuery("EXEC WZ_CustomMonsterReward '%s','%s','%d','%d','%d','%d','%d','%d'",lpMsg->account,lpMsg->name,lpMsg->MonsterClass,lpMsg->MapNumber,lpMsg->RewardValue1,lpMsg->RewardValue2, lpMsg->RewardValue3, lpMsg->RewardValue4);
	gQueryManager.Fetch();
	gQueryManager.Close();
}
...
{
	gQueryManager.ExecQuery("EXEC WZ_SetCoin '%s','%s','%d','%d','%d','%d'",lpMsg->account,lpMsg->name,lpMsg->value1,lpMsg->value2,lpMsg->value3, lpMsg->value4);
	gQueryManager.Fetch();
	gQueryManager.Close();
}
...
void GDCustomRankingRecv(SDHP_CUSTOM_RANKING_RECV* lpMsg, int index) // OK
{
	...
	if (gQueryManager.ExecQuery("EXEC WZ_CustomRanking %d", lpMsg->type) != 0)
	{
		while (gQueryManager.Fetch() != SQL_NO_DATA)
		{
			...
			gQueryManager.GetAsString("VALUE2", info.value2, sizeof(info.value2));
#if(CUSTOM_BRANKINGNEW)
			info.CheckOnline = gCharacterManager.CheckCharacterOnline(info.szName);
	...
	//	LogAdd(LOG_BLACK,"valor3 %s ",info.value3);
...
{
	if (gQueryManager.ExecQuery("SELECT Class,cLevel,ResetCount,Strength,Dexterity,Energy,Vitality,Leadership,Inventory FROM Character WHERE Name='%s'", lpMsg->NameChar) == 0 || gQueryManager.Fetch() == SQL_NO_DATA)
	{
		gQueryManager.Close();
		return;
	}
	else
	{
		...
		int Strength = (DWORD)gQueryManager.GetAsInteger("Strength");
		int Dexterity = (DWORD)gQueryManager.GetAsInteger("Dexterity");
		int Energy = (DWORD)gQueryManager.GetAsInteger("Energy");
		int Vitality = (DWORD)gQueryManager.GetAsInteger("Vitality");
		int Leadership = (DWORD)gQueryManager.GetAsInteger("Leadership");
		pMsg.TongPoint = (Strength + Dexterity + Energy + Vitality + Leadership);
		...
		gQueryManager.Close();
		//==GetMaster
		if (gQueryManager.ExecQuery("SELECT * FROM MasterSkillTree WHERE Name='%s'", lpMsg->NameChar) == 0 || gQueryManager.Fetch() == SQL_NO_DATA)
		{
			gQueryManager.Close();
			pMsg.MasterLevel = 0;
		}
		...
		gSocketManager.DataSend(index, (BYTE*)&pMsg, sizeof(pMsg));
		//LogAdd(LOG_RED, "Get Info Char Top (%s) ~ %s (%x)", pMsg.GuildName, pMsg.NameChar, pMsg.GuildMark);
	}
}
#endif
...
#if(NEW_POSTITEM)
void GDPostItemProc(BYTE* lpMsg, int size, int index)
{
	//LogAdd(LOG_BLACK, "Send to DS");
	if (size != sizeof(SDHP_POST_ITEM_SEND)) {
		return;
	}
	SDHP_POST_ITEM_SEND* Data = (SDHP_POST_ITEM_SEND*)lpMsg;


	SDHP_POST_ITEM_SEND pMsg = { 0, };

	pMsg.header.set(0x78, sizeof(pMsg));

	

	memcpy(pMsg.chatid, Data->chatid, sizeof(pMsg.chatid));
	...
	for (int i = 0; i < 12; i++)
	{
		pMsg.Item[i] = Data->Item[i];
	}

	pMsg.PeriodTime = Data->PeriodTime;
	strcat(pMsg.pre, Data->pre);
	strcat(pMsg.suf, Data->suf);

	pMsg.MapServerGroup = Data->MapServerGroup;
...
{
	int ItemBank[MAX_ITEM_BANK] = { 0 };

	switch (lpMsg->type)
	{
	case 0:		ItemBank[0] = lpMsg->count;		break;
	case 1:		ItemBank[1] = lpMsg->count;		break;
	case 2:		ItemBank[2] = lpMsg->count;		break;
	case 3:		ItemBank[3] = lpMsg->count;		break;
	case 4:		ItemBank[4] = lpMsg->count;		break;
	case 5:		ItemBank[5] = lpMsg->count;		break;
	case 6:		ItemBank[6] = lpMsg->count;		break;
	case 7:		ItemBank[7] = lpMsg->count;		break;
	case 8:		ItemBank[8] = lpMsg->count;		break;
	case 9:		ItemBank[9] = lpMsg->count;		break;
	case 20:	ItemBank[10] = lpMsg->count;		break;
	case 21:	ItemBank[11] = lpMsg->count;		break;
	case 22:	ItemBank[12] = lpMsg->count;		break;
	case 23:	ItemBank[13] = lpMsg->count;		break;
	case 24:	ItemBank[14] = lpMsg->count;		break;
	case 25:	ItemBank[15] = lpMsg->count;		break;
	case 26:	ItemBank[16] = lpMsg->count;		break;
	case 27:	ItemBank[17] = lpMsg->count;		break;
	case 28:	ItemBank[18] = lpMsg->count;		break;
	case 29:	ItemBank[19] = lpMsg->count;		break;
	case 30:	ItemBank[20] = lpMsg->count;		break;
	case 31:	ItemBank[21] = lpMsg->count;		break;
	case 32:	ItemBank[22] = lpMsg->count;		break;
	case 33:	ItemBank[23] = lpMsg->count;		break;
	case 34:	ItemBank[24] = lpMsg->count;		break;
	case 35:	ItemBank[25] = lpMsg->count;		break;
	case 36:	ItemBank[26] = lpMsg->count;		break;
	case 37:	ItemBank[27] = lpMsg->count;		break;
	case 38:	ItemBank[28] = lpMsg->count;		break;
	case 39:	ItemBank[29] = lpMsg->count;		break;
	}

	if (gQueryManager.ExecQuery("SELECT * FROM SPK_JewelBank WHERE AccountID='%s'", lpMsg->account) == 0 || gQueryManager.Fetch() == SQL_NO_DATA)
	{
		gQueryManager.Close();
		gQueryManager.ExecQuery("INSERT INTO SPK_JewelBank (AccountID,ItemBank0,ItemBank1,ItemBank2,ItemBank3,ItemBank4,ItemBank5,ItemBank6,ItemBank7,ItemBank8,ItemBank9,ItemBank10,ItemBank11,ItemBank12,ItemBank13,ItemBank14,ItemBank15,ItemBank16,ItemBank17,ItemBank18,ItemBank19,ItemBank20,ItemBank21,ItemBank22,ItemBank23,ItemBank24,ItemBank25,ItemBank26,ItemBank27,ItemBank28,ItemBank29) VALUES ('%s',%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d)", lpMsg->account, ItemBank[0], ItemBank[1], ItemBank[2], ItemBank[3], ItemBank[4], ItemBank[5], ItemBank[6], ItemBank[7], ItemBank[8], ItemBank[9], ItemBank[10], ItemBank[11], ItemBank[12], ItemBank[13], ItemBank[14], ItemBank[15], ItemBank[16], ItemBank[17], ItemBank[18], ItemBank[19], ItemBank[20], ItemBank[21], ItemBank[22], ItemBank[23], ItemBank[24], ItemBank[25], ItemBank[26], ItemBank[27], ItemBank[28], ItemBank[29]);
		gQueryManager.Close();
	...
	{
		gQueryManager.Close();
		gQueryManager.ExecQuery("Update SPK_JewelBank SET ItemBank0+=%d,ItemBank1+=%d,ItemBank2+=%d,ItemBank3+=%d,ItemBank4+=%d,ItemBank5+=%d,ItemBank6+=%d,ItemBank7+=%d,ItemBank8+=%d,ItemBank9+=%d, ItemBank10+=%d, ItemBank11+=%d, ItemBank12+=%d, ItemBank13+=%d, ItemBank14+=%d, ItemBank15+=%d, ItemBank16+=%d, ItemBank17+=%d, ItemBank18+=%d, ItemBank19+=%d,ItemBank20+=%d,ItemBank21+=%d,ItemBank22+=%d,ItemBank23+=%d,ItemBank24+=%d,ItemBank25+=%d,ItemBank26+=%d,ItemBank27+=%d,ItemBank28+=%d,ItemBank29+=%d WHERE AccountID = '%s'", ItemBank[0], ItemBank[1], ItemBank[2], ItemBank[3], ItemBank[4], ItemBank[5], ItemBank[6], ItemBank[7], ItemBank[8], ItemBank[9], ItemBank[10], ItemBank[11], ItemBank[12], ItemBank[13], ItemBank[14], ItemBank[15], ItemBank[16], ItemBank[17], ItemBank[18], ItemBank[19], ItemBank[20], ItemBank[21], ItemBank[22], ItemBank[23], ItemBank[24], ItemBank[25], ItemBank[26], ItemBank[27], ItemBank[28], ItemBank[29], lpMsg->account);
		gQueryManager.Close();
	}
}
...
void GDCustomJewelBankInfoRecv(SDHP_CUSTOM_JEWELBANK_INFO_RECV* lpMsg, int index)
{
	...
	{
		for (int M = 0; M < MAX_ITEM_BANK; M++)
		{
			char SQLSlot[30];
			sprintf_s(SQLSlot, "ItemBank%d", M);
			pMsg.ItemBank[M] = gQueryManager.GetAsInteger(SQLSlot);
		}
