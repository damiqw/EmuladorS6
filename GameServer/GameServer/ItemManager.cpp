...
char* CItemManager::GetItemName1(int index, int level) // OK
{ 
	...
	for (std::map<int, ITEM_LEVEL_INFO>::iterator it = gItemLevel.m_ItemLevelInfo.begin(); it != gItemLevel.m_ItemLevelInfo.end(); it++)
	{
...
BYTE CItemManager::InventoryAddItem(int aIndex,CItem item,int slot) // OK
{
	...
	gInvoker.InvokerSummon(lpObj->Index);
...
void CItemManager::UpdateInventoryViewport(int aIndex,int slot) // OK
{
	...
	if(slot == 10 || slot == 11)
	{
		...
		if (gInvoker.InvokerSummon(aIndex) != 0) //FIX
		{
			gObjViewportListProtocolCreate(&gObj[aIndex]);
			gObjectManager.CharacterUpdateMapEffect(&gObj[aIndex]);
		}
...
void CItemManager::CGItemGetRecv(PMSG_ITEM_GET_RECV* lpMsg,int aIndex) // OK
{
	...
	if (gQuestWorldObjective.CheckQuestWorldObjectiveItemCount(lpObj, lpItem->m_Index, lpItem->m_Level) == 0)
	{
		DataSend(aIndex, (BYTE*)&pMsg, pMsg.header.size);
		return;
	}
	...
	if (gObj[aIndex].AutoChange == true)
	{
		g_AutoChangeCmd.AutoChange(aIndex);
	}
}
...
void CItemManager::CGItemUseRecv(PMSG_ITEM_USE_RECV* lpMsg,int aIndex) // OK
{
	...
	{
		if(gKalima.CreateKalimaGate(aIndex,gEventEntryLevel.GetKALevel(lpObj),lpObj->X,lpObj->Y) != 0)
		{
	...
	else if (lpItem->m_Index == GET_ITEM(14, 151)) // Lv. 1 Letter of Request
	{
		if (gQuestWorld.CharacterUseLetter(lpObj) != 0)
		{
			this->InventoryDelItem(aIndex, lpMsg->SourceSlot);
			this->GCItemDeleteSend(aIndex, lpMsg->SourceSlot, 1);
		}
	...
	else if (lpItem->m_Index == GET_ITEM(14, 170)) // Lv. 2 Letter of Request
	{
		if (gQuestWorld.CharacterUseLetter(lpObj) != 0)
		{
			this->InventoryDelItem(aIndex, lpMsg->SourceSlot);
...
{
	if (gServerInfo.m_Disable_BuyForWC == 0)
	{
		gCashShop.GDCashShopRecievePointSend(aIndex, (DWORD)&CItemManager::CGItemBuyRecvCallback, lpMsg->slot, 0);
	}
	else
	{
		LPOBJ lpObj = &gObj[aIndex];
		...
		if (ok == 0 && gServerInfo.m_CustomShopMessageBox > 0)
		{
			...
			if (gServerInfo.m_CustomShopMessageBox == 1 && (item.m_BuyWCoinC > 0 || item.m_BuyWCoinP > 0 || item.m_BuyWCoinG > 0 || item.m_BuyRuud > 0))
			{
				DataSend(aIndex, (BYTE*)&pMsg, pMsg.header.size);
...
void CItemManager::CGItemBuyRecvCallback(LPOBJ lpObj, BYTE slot, DWORD null, DWORD WCoinC, DWORD WCoinP, DWORD GoblinPoint,DWORD Ruud)
{
	...
	if (gShopManager.GetItemByIndex(lpObj->TargetShopNumber, &item, slot) == 0)
	{
		DataSend(lpObj->Index, (BYTE*)&pMsg, pMsg.header.size);
		return;
	}
	...
	else if (item.m_PriceType == WCOINC)
	{
		if (WCoinC < item.m_BuyWCoinC)
		{
			gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "You do not have enough %s!", gMessage.GetMessage(660));
			DataSend(lpObj->Index, (BYTE*)&pMsg, pMsg.header.size);
			return;
		}
	}
	else if (item.m_PriceType == WCOINP)
	{
		if (WCoinP < item.m_BuyWCoinP)
		{
			gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "You do not have enough %s!", gMessage.GetMessage(661));
			DataSend(lpObj->Index, (BYTE*)&pMsg, pMsg.header.size);
			return;
		}
	}
	else if (item.m_PriceType == WCOING)
	{
		if (GoblinPoint < item.m_BuyWCoinG)
		{
			gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "You do not have enough %s!", gMessage.GetMessage(662));
			DataSend(lpObj->Index, (BYTE*)&pMsg, pMsg.header.size);
			return;
		}
	}
	else if (item.m_PriceType == RUUD)
	{
		if (GoblinPoint < item.m_BuyRuud)
		{
			gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "You do not have enough %s!", gMessage.GetMessage(663));
			DataSend(lpObj->Index, (BYTE*)&pMsg, pMsg.header.size);
			return;
		}
	}
	...
	if (item.m_Index == GET_ITEM(13, 4) || item.m_Index == GET_ITEM(13, 5)) //Dark Horse,Dark Reaven
	{
		GDCreateItemSend(lpObj->Index, 0xEB, 0, 0, item.m_Index, (BYTE)item.m_Level, (BYTE)item.m_Durability, item.m_Option1, item.m_Option2, item.m_Option3, -1, item.m_NewOption, item.m_SetOption, item.m_JewelOfHarmonyOption, item.m_ItemOptionEx, item.m_SocketOption, item.m_SocketOptionBonus, 0);
		if (item.m_PriceType == ZEN)
		{
			lpObj->Money -= item.m_BuyMoney + tax;
		...
		{
			gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "You buy item for %d %s. Balance: %d", item.m_BuyWCoinC, gMessage.GetMessage(660), WCoinC - item.m_BuyWCoinC);
			gCashShop.GDCashShopSubPointSaveSend(lpObj->Index, 0, item.m_BuyWCoinC, 0, 0, 0);
			//gLog.Output(LOG_ITEMVALUE_BUY, "[%s][%s] Buy item %d [%d] for %d WCoinC. Balanñe %d", lpObj->Account, lpObj->Name, item.m_Index, item.m_Serial, item.m_BuyWCoinC, WCoinC);
		}
		else if (item.m_PriceType == WCOINP)
		{
			gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "You buy item for %d %s. Balance: %d", item.m_BuyWCoinP, gMessage.GetMessage(661), WCoinP - item.m_BuyWCoinP);
			gCashShop.GDCashShopSubPointSaveSend(lpObj->Index, 0, 0, item.m_BuyWCoinP, 0, 0);
			//gLog.Output(LOG_ITEMVALUE_BUY, "[%s][%s] Buy item %d [%d] for %d WCoinP. Balanñe %d", lpObj->Account, lpObj->Name, item.m_Index, item.m_Serial, item.m_BuyWCoinP, WCoinP);
		}
		else if (item.m_PriceType == WCOING)
		{
			gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "You buy item for %d %s. Balance: %d", item.m_BuyWCoinG, gMessage.GetMessage(662), GoblinPoint - item.m_BuyWCoinG);
			gCashShop.GDCashShopSubPointSaveSend(lpObj->Index, 0, 0, 0, item.m_BuyWCoinG,0);
			//gLog.Output(LOG_ITEMVALUE_BUY, "[%s][%s] Buy item %d [%d] for %d GoblinPoints. Balanñe %d", lpObj->Account, lpObj->Name, item.m_Index, item.m_Serial, item.m_BuyWCoinG, GoblinPoint);
		}
		else if (item.m_PriceType == RUUD)
		{
			gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "You buy item for %d %s. Balance: %d", item.m_BuyWCoinG, gMessage.GetMessage(663), Ruud - item.m_BuyRuud);
			gCashShop.GDCashShopSubPointSaveSend(lpObj->Index, 0, 0, 0,0, item.m_BuyRuud);
			//gLog.Output(LOG_ITEMVALUE_BUY, "[%s][%s] Buy item %d [%d] for %d Ruud. Balanñe %d", lpObj->Account, lpObj->Name, item.m_Index, item.m_Serial, item.m_BuyWCoinG, m_BuyRuud);
		}
	...
	if (item.m_Index >= GET_ITEM(13, 71) && item.m_Index <= GET_ITEM(13, 75)) // Random Item
	{
		...
		{
			gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "You buy item for %d %s. Balance: %d", item.m_BuyRuud, gMessage.GetMessage(663), Ruud - item.m_BuyRuud);
			gCashShop.GDCashShopSubPointSaveSend(lpObj->Index, 0, 0, 0,0, item.m_BuyRuud);
			//gLog.Output(LOG_ITEMVALUE_BUY, "[%s][%s] Buy item %d [%d] for %d Ruud. Balanñe %d", lpObj->Account, lpObj->Name, item.m_Index, item.m_Serial, item.m_BuyWCoinG, m_BuyRuud);
		}
	...
	if (gItemManager.InventoryInsertItemStack(lpObj, &item) != 0)
	{
		...
		{
			gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "You buy item for %d %s. Balance: %d", item.m_BuyRuud, gMessage.GetMessage(663), Ruud - item.m_BuyRuud);
			gCashShop.GDCashShopSubPointSaveSend(lpObj->Index, 0, 0, 0,0, item.m_BuyWCoinG);
			//gLog.Output(LOG_ITEMVALUE_BUY, "[%s][%s] Buy item %d [%d] for %d Ruud. Balanñe %d", lpObj->Account, lpObj->Name, item.m_Index, item.m_Serial, item.m_BuyWCoinG, m_BuyRuud);
		}
	...
	else if (item.m_PriceType == WCOING)
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "You buy item for %d %s. Balance: %d", item.m_BuyWCoinG, gMessage.GetMessage(662), GoblinPoint - item.m_BuyWCoinG);
		gCashShop.GDCashShopSubPointSaveSend(lpObj->Index, 0, 0, 0, item.m_BuyWCoinG, 0);
		//gLog.Output(LOG_ITEMVALUE_BUY, "[%s][%s] Buy item %d [%d] for %d GoblinPoints. Balance %d", lpObj->Account, lpObj->Name, item.m_Index, item.m_Serial, item.m_BuyWCoinG, GoblinPoint);
	}
	else if (item.m_PriceType == RUUD)
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "You buy item for %d %s. Balance: %d", item.m_BuyRuud, gMessage.GetMessage(663), Ruud - item.m_BuyRuud);
		gCashShop.GDCashShopSubPointSaveSend(lpObj->Index, 0, 0, 0,0, item.m_BuyRuud);
		//gLog.Output(LOG_ITEMVALUE_BUY, "[%s][%s] Buy item %d [%d] for %d Ruud. Balance %d", lpObj->Account, lpObj->Name, item.m_Index, item.m_Serial, item.m_BuyRuud, Ruud);
	}
...
void CItemManager::CGItemSellRecv(PMSG_ITEM_SELL_RECV* lpMsg,int aIndex) // OK
{
	...
	if (gServerInfo.m_BlockSellShopPeriodicItem != 0 && lpItem->m_IsPeriodicItem != 0)
	{
		DataSend(aIndex,(BYTE*)&pMsg,pMsg.header.size);
		return;
	}
...
int CItemManager::GetInventoryItemSpace(LPOBJ lpObj, int index) // OK
{
    ITEM_INFO ItemInfo;
...
bool CItemManager::CheckItemMoveToEquip(LPOBJ lpObj, CItem* lpItem, int slot) //ZEUS UP20
{
...
void CItemManager::CGItemEquipRepairRecv(PMSG_ITEM_REPAIR_RECV* lpMsg, int aIndex) //ZEUS UP20
{
	LPOBJ lpObj = &gObj[aIndex];
...
BYTE CItemManager::MoveItemToEquipFromInventory(LPOBJ lpObj, BYTE SourceSlot, BYTE TargetSlot, BYTE TargetFlag) //ZEUS UP20
{
	if (INVENTORY_FULL_RANGE(SourceSlot) == 0)
	{
		return 0xFF;
...
BYTE CItemManager::MoveItemToEquipFromChaosBox(LPOBJ lpObj, BYTE SourceSlot, BYTE TargetSlot, BYTE TargetFlag) //ZEUS UP20
{
...
BYTE CItemManager::MoveItemToPersonalShopFromEquip(LPOBJ lpObj, BYTE SourceSlot, BYTE TargetSlot, BYTE TargetFlag) //ZEUS UP20
{
...
BYTE CItemManager::MoveItemToEquipFromPersonalShop(LPOBJ lpObj, BYTE SourceSlot, BYTE TargetSlot, BYTE TargetFlag) //ZEUS UP20
{
	if (INVENTORY_SHOP_RANGE(SourceSlot) == 0)
	{
		return 0xFF;
...
BYTE CItemManager::MoveItemToEquipFromTrade(LPOBJ lpObj, BYTE SourceSlot, BYTE TargetSlot, BYTE TargetFlag) //ZEUS UP20
{
	if (TRADE_RANGE(SourceSlot) == 0)
	{
		return 0xFF;
...
BYTE CItemManager::MoveItemToInventoryFromEquip(LPOBJ lpObj, BYTE SourceSlot, BYTE TargetSlot, BYTE TargetFlag) //ZEUS UP20
{
	if (EQUIP_INVENTORY_RANGE(SourceSlot) == 0)
	{
		return 0xFF;
...
BYTE CItemManager::MoveItemToChaosBoxFromEquip(LPOBJ lpObj, BYTE SourceSlot, BYTE TargetSlot, BYTE TargetFlag) //ZEUS UP20
{
	if (EQUIP_INVENTORY_RANGE(SourceSlot) == 0)
	{
		return 0xFF;
...
BYTE CItemManager::MoveItemToTradeFromEquip(LPOBJ lpObj, BYTE SourceSlot, BYTE TargetSlot, BYTE TargetFlag) //ZEUS UP20
{
	if (EQUIP_INVENTORY_RANGE(SourceSlot) == 0)
	{
		return 0xFF;
...
void CItemManager::CGMoveItemProc(PMSG_MOVEITEM* aRecv, short aIndex)
{
	...
	else if (aRecv->tFlag == 0 && aRecv->sFlag == 0) //equip/unequip item
	{
		...
		{ //wear equipment case
			...
			else if (pMsg.TargetSlot == 13
				&& lpObj->EquipInventory[13].IsItem() //slot 13 already has item
				&& !lpObj->EquipInventory[14].IsItem()) //slot 14 empty earring
			{
				pMsg.TargetSlot = 14;
			}
...
#if(HAISLOTRING)
	if (pMsg.TargetSlot == 12 || pMsg.TargetSlot == 13 || pMsg.TargetSlot == 14)
	{
		pMsg.TargetFlag = 40;
	}
#endif
