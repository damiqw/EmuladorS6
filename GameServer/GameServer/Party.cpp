...
void CParty::CGPartyRequestRecv(PMSG_PARTY_REQUEST_RECV* lpMsg,int aIndex) // OK
{
	...
	// ----
	if( TotalLevel > gObj[bIndex].Level && TotalLevel - gObj[bIndex].Level > gServerInfo.m_PartyMaxGapLevel )
	{	
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "You can't create party with %s (%s need %d more levels)", gObj[bIndex].Name, gObj[bIndex].Name, TotalLevel - gServerInfo.m_PartyMaxGapLevel - gObj[bIndex].Level);
		this->GCPartyResultSend(aIndex,5);
		return;
	}
	// ----
	if( TotalLevel < gObj[bIndex].Level && gObj[bIndex].Level - TotalLevel > gServerInfo.m_PartyMaxGapLevel )
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "You can't create party with %s (need %d more levels)", gObj[bIndex].Name, gObj[bIndex].Level - gServerInfo.m_PartyMaxGapLevel - TotalLevel);
		this->GCPartyResultSend(aIndex,5);
		return;
	}	
