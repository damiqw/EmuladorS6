...
void gObjViewportProc() // OK
{
	...
	for (int n = OBJECT_START_USER; n < MAX_OBJECT; n++)
	{
		gCB_MapUsageTime.CheckMapUsageTime(n);
	}
	#endif
...
void gObjFirstProc() // OK
{
	...
	for(int n=OBJECT_START_USER;n < MAX_OBJECT;n++)
	{
		...
		if (Coin1 > 0 || Coin2 > 0 || Coin3 > 0)
			GDSetCoinSend(lpObj->Index, Coin1, Coin2, Coin3,0,"OnlineReward"); 
	}
}
...
void gObjAllLogOut() // OK
{
	...
	for(int n=OBJECT_START_USER;n < MAX_OBJECT;n++)
	{
		if(gObj[n].Connected != OBJECT_OFFLINE && gObj[n].Type == OBJECT_USER)
		{
			...
			s_FakeOnline.OnAttackAlreadyConnected(&gObj[n]); // add this
#endif
...
#if USE_FAKE_ONLINE == TRUE
			s_FakeOnline.OnAttackAlreadyConnected(&gObj[n]); // this
#endif
...
void gObjCharZeroSet(int aIndex) // OK
{
	...
	memset(lpObj->ServerName,0,sizeof(lpObj->ServerName));
...
#if(HAISLOTRING)
	gObjSetEquipInventory1Pointer(lpObj); //ZEUS UP20
#endif
	...
	lpObj->WarehouseState = 0;
	lpObj->WarehouseQtd = 0;
...
//	lpObj->AttackCustomOfflineMoneyDelay = 0;
	...
	#if(MAP_USAGE)
	ZeroMemory(lpObj->MapUsageTime, sizeof(lpObj->MapUsageTime));
...
#if(HAISLOTRING)
	gObjFixEquipInventoryPointer(aIndex);
#endif
...
bool gObjFixEquipInventoryPointer(int aIndex) //ZEUS UP20
{
	if (OBJECT_RANGE(aIndex) == 0)
	{
		return 0;
...
#if(HAISLOTRING)
	memcpy(lpObj->EquipInventoryMap2, lpObj->EquipInventoryMap1, EQUIP_INVENTORY_SIZE);
#endif
...
#if(HAISLOTRING)
	memcpy(lpObj->EquipInventoryMap1, lpObj->EquipInventoryMap2, EQUIP_INVENTORY_SIZE);
#endif
...
void gObjUserDie(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	{
		gCustomDeathReward.CheckRewardZone(lpObj, lpTarget);
	}
...
void gObjPlayerKiller(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	if (!gPKZone.CheckZoneMurderLevel(lpObj, lpTarget)) 
	{
		return;
	}
	...
	for (n = 0; n < MAX_OBJECT; n++)
	{
		...
		if (lpObj->Connected > OBJECT_LOGGED)
		{
			...
			if (lpObj->Type == OBJECT_USER)
			{
				...
				gInvasionManager.CheckRegenTimeMonter();
#endif
...
#if(LuckyWheelV2)
				gCustomVongQuay.MakeItem(lpObj->Index, 1);
#endif
#if(GAMESERVER_TYPE==1)
