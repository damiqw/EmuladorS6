...
void CSItemOption::ItemTooltipS15(float PosX, float PosY, float Width, float Height, float Arg5, int Arg6)
{
	...
	if (gProtect.m_MainInfo.ItemTooltipUpgrade == 0)
	{
		glEnable(0xDE1u);
...
void ItemTooltipRender(int x, int y, int nCount, int witd, int height, int type, int Align)
{
	...
	if (Align == 1 && nCount > 0)
	{
		...
		if (gInterface.item_post_ && pCursorX < 180)
		{
			gInterface.DrawItemIMG(cRenderX + (cRenderW / 2) - ((SellItem - 20) / 2), cRenderY, SellItem - 20, SellItem - 20, gObjUser.PostItemID, gObjUser.PostItemLV, gObjUser.PostItemExc, gObjUser.PostItemAcc, 0);
		}
	}
	//-- Escritura
