...
void InitWing() // OK
{
	...
	SetCompleteHook(0xE8, 0x58B78F, &GetPrice);
	SetCompleteHook(0xE8, 0x58B7A6, &GetPrice);
	SetCompleteHook(0xE8, 0x58B7FC, &GetPrice);
	SetCompleteHook(0xE8, 0x5C1A7F, &GetPrice);
	SetCompleteHook(0xE8, 0x5C2117, &GetPrice);
	SetCompleteHook(0xE8, 0x5C4268, &GetPrice);
	SetCompleteHook(0xE8, 0x7AF956, &GetPrice);
	SetCompleteHook(0xE8, 0x7E4449, &GetPrice);
	SetCompleteHook(0xE8, 0x7E4497, &GetPrice);
	SetCompleteHook(0xE8, 0x7E44EC, &GetPrice);
	SetCompleteHook(0xE8, 0x846CE9, &GetPrice);
	SetCompleteHook(0xE8, 0x846D63, &GetPrice);
	SetCompleteHook(0xE8, 0x847713, &GetPrice);
	SetCompleteHook(0xE8, 0x968550, &GetPrice);
...
void ItemModelLoad() // OK
{
	...
	//-------------------- custom pet
	for(std::map<int,CUSTOM_PET_INFO>::iterator it = g_CustomPet.m_CustomPetInfo.begin();it != g_CustomPet.m_CustomPetInfo.end();it++)
	{
		#if DEBUG_PRO == 0
		LoadItemModel((it->second.ItemIndex + ITEM_BASE_MODEL),"Item\\",it->second.ModelName,-1);
		#else
		LoadItemModel((it->second.ItemIndex + ITEM_BASE_MODEL),"Item\\Muun\\",it->second.ModelName,-1);
		#endif
	}

	for (int i = 0; i < MAX_CUSTOM_FLAG; i++)
	{
		if (gCustomFlag.m_CustomFlagInfo[i].Index != -1)
		{
			LoadItemModel(gCustomFlag.m_CustomFlagInfo[i].ItemIndex + ITEM_BASE_MODEL, "Custom\\CustomFlag\\", gCustomFlag.m_CustomFlagInfo[i].ModelName, -1);
		}
	}
}
...
void ItemTextureLoad() // OK
{
	...
	for(std::map<int,CUSTOM_PET_INFO>::iterator it = g_CustomPet.m_CustomPetInfo.begin();it != g_CustomPet.m_CustomPetInfo.end();it++)
	{
		...
		LoadItemTexture((it->second.ItemIndex + ITEM_BASE_MODEL),"Item\\",it->second.ModelName,-1);
		#else
		LoadItemTexture((it->second.ItemIndex + ITEM_BASE_MODEL),"Item\\Muun\\",it->second.ModelName,-1);
		#endif
	}
	...
	for (int i = 0; i < MAX_CUSTOM_FLAG; i++)
	{
		if (gCustomFlag.m_CustomFlagInfo[i].Index != -1)
		{
			LoadItemTexture(gCustomFlag.m_CustomFlagInfo[i].ItemIndex + ITEM_BASE_MODEL, "Custom\\CustomFlag\\", gCustomFlag.m_CustomFlagInfo[i].ModelName, -1);
		}
	}
}
...
__int64	GetPrice(ObjectItem* lpItem, int Mode)
{
	__int64 Money = -1;
