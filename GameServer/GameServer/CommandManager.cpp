...
bool CCommandManager::ManagementCore(LPOBJ lpObj,char* message, int Npc) // OK
{
	...
	switch(code)
	{
		...
		case COMMAND_BLOCCHAR:
			CommandBanChar(lpObj,argument);
			Result = 0;
			break;
		case COMMAND_BLOCACC:
			CommandBanAcc(lpObj,argument);
			Result = 0;
			break;
		...
		case COMMAND_CUSTOM_MARRIAGE4:
			...
			Result = 0;
			break;
		case COMMAND_STARTLUKEHELPER:
			Result = CommandStartLukeHelper(lpObj,argument);
			break;
		case COMMAND_ITEM_CHAGE:
			Result = CommandItemChage(lpObj);
			break;
		case COMMAND_AUTO_CHAGE:
			Result = CommandAutoChage(lpObj);
			break;
		default:
			return 0;
	}
...
void CCommandManager::RemoveRequisites(LPOBJ lpObj, int IdCommand)
{
	...
	if(gCommand.GetInfo(IdCommand, &CommandData) == 1)
	{
		...
		if (CommandData.Coin1 > 0 || CommandData.Coin2 > 0 || CommandData.Coin3 > 0)
		{
			GDSetCoinSend(lpObj->Index, -(CommandData.Coin1), -(CommandData.Coin2), -(CommandData.Coin3), -(0), "Command");
		}
	}
}
...
bool CCommandManager::CommandPost(LPOBJ lpObj,char* arg) // OK
{
	...
	{
		GDGlobalPostSend(gMapServerManager.GetMapServerGroup(),0,lpObj->Name,arg,lpObj->ServerName);
	}
	else if(gServerInfo.m_CommandPostType == 5)
	{
		GDGlobalPostSend(gMapServerManager.GetMapServerGroup(),1,lpObj->Name,arg,lpObj->ServerName);
	}
	else if(gServerInfo.m_CommandPostType == 6)
	{
		GDGlobalPostSend(gMapServerManager.GetMapServerGroup(),2,lpObj->Name,arg,lpObj->ServerName);
	}
	else if(gServerInfo.m_CommandPostType == 7)
	{
		GDGlobalPostSend(gMapServerManager.GetMapServerGroup(),3,lpObj->Name,arg,lpObj->ServerName);
	}

	gLog.Output(LOG_COMMAND,"[CommandPost][%s][%s] - (Message: %s - %s)",lpObj->Account,lpObj->Name,lpObj->ServerName,arg);
...
bool CCommandManager::CommandItemChage(LPOBJ lpObj) // OK
{
	int iResult = g_ItemChangeCmd.ItemChange(lpObj->Index);
	...
	if (iResult == 0)
	{
		//MsgOutput(lpObj->m_Index, Lang.GetText(0,639));
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(992));
	}
	else
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(993));
	}

	return 1;
}

bool CCommandManager::CommandAutoChage(LPOBJ lpObj) // OK
{
	if (lpObj->AutoChange == false)
	{
		lpObj->AutoChange = true;

		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(994));
	}
	else
	{
		lpObj->AutoChange = false;

		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(995));
	}
	return 1;
}
...
{
	if(gServerInfo.m_CommandWareType > 0)
	{
		return 0;
	}
...
void CCommandManager::DGCommandResetRecv(SDHP_COMMAND_RESET_RECV* lpMsg) // OK
{
	...
	if(gServerInfo.m_CommandResetType == 1)
	{
		...
		point += ((gQuest.CheckQuestListState(lpObj,2,QUEST_FINISH)==0)?0:((lpObj->Level>gServerInfo.m_PlusStatMinLevel)?((lpObj->Level-220)*gServerInfo.m_PlusStatPoint):0));
	...
	if(gServerInfo.m_CommandResetRewardCoin1Value[lpObj->AccountLevel] > 0 || gServerInfo.m_CommandResetRewardCoin2Value[lpObj->AccountLevel] > 0 || gServerInfo.m_CommandResetRewardCoin3Value[lpObj->AccountLevel] > 0 || gServerInfo.m_CommandResetRewardRuudValue[lpObj->AccountLevel] > 0)
	{
			GDSetCoinSend(lpObj->Index,+(gServerInfo.m_CommandResetRewardCoin1Value[lpObj->AccountLevel]),+(gServerInfo.m_CommandResetRewardCoin2Value[lpObj->AccountLevel]),+(gServerInfo.m_CommandResetRewardCoin3Value[lpObj->AccountLevel]), +(gServerInfo.m_CommandResetRewardRuudValue[lpObj->AccountLevel]),"ResetReward");
	
			gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(252),gServerInfo.m_CommandResetRewardCoin1Value[lpObj->AccountLevel],gServerInfo.m_CommandResetRewardCoin2Value[lpObj->AccountLevel],gServerInfo.m_CommandResetRewardCoin3Value[lpObj->AccountLevel], gServerInfo.m_CommandResetRewardRuudValue[lpObj->AccountLevel]);
...
void CCommandManager::DGCommandMasterResetRecv(SDHP_COMMAND_MASTER_RESET_RECV* lpMsg) // OK
{
	...
	if(gServerInfo.m_CommandMasterResetRewardCoin1Value[lpObj->AccountLevel] > 0 || gServerInfo.m_CommandMasterResetRewardCoin2Value[lpObj->AccountLevel] > 0 || gServerInfo.m_CommandMasterResetRewardCoin3Value[lpObj->AccountLevel] > 0 || gServerInfo.m_CommandMasterResetRewardRuudValue[lpObj->AccountLevel] > 0)
	{
			GDSetCoinSend(lpObj->Index,+(gServerInfo.m_CommandMasterResetRewardCoin1Value[lpObj->AccountLevel]),+(gServerInfo.m_CommandMasterResetRewardCoin2Value[lpObj->AccountLevel]),+(gServerInfo.m_CommandMasterResetRewardCoin3Value[lpObj->AccountLevel]), +(gServerInfo.m_CommandMasterResetRewardRuudValue[lpObj->AccountLevel]),"MasterResetReward");
	
			gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(252),gServerInfo.m_CommandMasterResetRewardCoin1Value[lpObj->AccountLevel],gServerInfo.m_CommandMasterResetRewardCoin2Value[lpObj->AccountLevel],gServerInfo.m_CommandMasterResetRewardCoin3Value[lpObj->AccountLevel], gServerInfo.m_CommandMasterResetRewardRuudValue[lpObj->AccountLevel]);
...
bool CCommandManager::CommandRename(LPOBJ lpObj,char* arg) // OK
{
	...
	if(CheckSpecialText(RenameName) == false)
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,"Please use only letters and numbers!");
		return 0;
	}
...
void CCommandManager::CommandBanChar(LPOBJ lpObj,char* arg) // OK
{
	...
	gNotice.GCNoticeSend(lpTarget->Index,1,0,0,0,0,0,gMessage.GetMessage(628));

	gObjUserKill(lpTarget->Index);

	gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(986),lpTarget->Name);
...
void CCommandManager::DGCommandBanCharacterRecv(SDHP_COMMAND_BAN_CHARACTER_RECV* lpMsg) // OK
{
	...
	if (lpMsg->result != 0)
	{
		if (lpMsg->CtlCode == 0)
		{
			gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(987),lpMsg->name);
...
void CCommandManager::DGCommandBanAccountRecv(SDHP_COMMAND_BAN_ACCOUNT_RECV* lpMsg) // OK
{
	...
	if(lpMsg->result != 0)
	{
		if(lpMsg->BlocCode == 0)
		{
			gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(987),lpMsg->account);
...
bool CCommandManager::CommandStartLukeHelper(LPOBJ lpObj,char* arg) // OK
{
	gCustomLukeHelper.StartLukeHelper();
	gLog.Output(LOG_COMMAND,"[CommandStartLukeHelper][%s][%s] - Used",lpObj->Account,lpObj->Name);
...
bool CCommandManager::CommandStartInvasion(LPOBJ lpObj,char* arg) // OK
{
	...
	gInvasionManager.ForceStart(index);
...
bool CCommandManager::CommandSetCoin(LPOBJ lpObj,char* arg) // OK
{
	...
	if (coin==1)
	{
		GDSetCoinSend(lpTarget->Index, value, 0, 0,0,"GmSetCoin");
		gLog.Output(LOG_COMMAND,"[CommandSetCoin][%s][%s] - (Coin: %d, Value: %d)",lpObj->Account,lpObj->Name,coin,value);
	}
	if (coin==2)
	{
		GDSetCoinSend(lpTarget->Index, 0, value, 0,0,"GmSetCoin");
		gLog.Output(LOG_COMMAND,"[CommandSetCoin][%s][%s] - (Coin: %d, Value: %d)",lpObj->Account,lpObj->Name,coin,value);
	}
	if (coin==3)
	{
		GDSetCoinSend(lpTarget->Index, 0, 0, value,0,"GmSetCoin");
		gLog.Output(LOG_COMMAND,"[CommandSetCoin][%s][%s] - (Coin: %d, Value: %d)",lpObj->Account,lpObj->Name,coin,value);
	}
	if (coin == 4)
	{
		GDSetCoinSend(lpTarget->Index, 0, 0,0, value, "GmSetCoin");
		gLog.Output(LOG_COMMAND, "[CommandSetCoin][%s][%s] - (Coin: %d, Value: %d)", lpObj->Account, lpObj->Name, coin, value);
	}
