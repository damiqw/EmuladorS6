...
void CImperialGuardian::SetState_PLAY4() // OK
{
	...
	if(this->m_Level == 3)
	{
		...
		this->m_MaxMonster = 100;
	}

	this->m_RemainTime = this->m_EventTime*80000;
		...
		if(OBJECT_RANGE(this->m_User[n].Index) != 0)
		{
			if(this->m_Level == 3)
			{
				if(this->m_Stage == 3 && this->m_CurMonster >= this->m_MaxMonster)
				{
					...
					gShopPointEx.AddEventBonus(this->m_User[n].Index, ShopPointExEvent::IG);
...
bool CImperialGuardian::AddUser(int aIndex) // OK
{
	...
	for(int n=0;n < MAX_IG_USER;n++)
	{
		...
		gObjEventEntrySaveSend(&gObj[aIndex],5);
		return 1;
	}
...
void CImperialGuardian::CGImperialGuardianEnterRecv(PMSG_IMPERIAL_GUARDIAN_ENTER_RECV* lpMsg,int aIndex) // OK
{
	...
	if(gServerInfo.m_ImperialGuardianMaxEntryCount[lpObj->AccountLevel] != 0 && lpObj->IGCount >= gServerInfo.m_ImperialGuardianMaxEntryCount[lpObj->AccountLevel])
	{
		GCNewMessageSend(aIndex,0,0,gMessage.GetMessage(918),gServerInfo.m_ImperialGuardianMaxEntryCount[lpObj->AccountLevel]);
		return;
	}
