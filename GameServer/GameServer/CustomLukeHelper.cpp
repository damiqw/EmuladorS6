...
void CCustomLukeHelper::MainProc() // OK
{
	if((GetTickCount()-this->m_TickCount) >= 1000)
	{
		...
		if (m_CustomLukeHelperSwitch == 0)
		{
			if (gServerDisplayer.LukeHelper != -1)
			{
				gServerDisplayer.LukeHelper = -1;
			}
		}
		else
		{
			if (this->m_State == CUSTOM_LUKE_HELPER_EMPTY)
			{
				gServerDisplayer.LukeHelper = this->m_RemainTime;
			}
			else
			{
				if (gServerDisplayer.LukeHelper != 0)
				{
					gServerDisplayer.LukeHelper = 0;
				}
			}
		}
...
void CCustomLukeHelper::ProcState_EMPTY() // OK
{
	...
	if(this->m_RemainTime <= 0)
	{
		...
		gNotice.GCNoticeSendToAll(0,0,0,0,0,0,gMessage.GetMessage(541),gMapManager.GetMapName(lpInfo->Map));
		gNotice.GCNoticeSendToAll(0,0,0,0,0,0,gMessage.GetMessage(542));
...
void CCustomLukeHelper::ProcState_START() // OK
{
	if(this->m_RemainTime <= 0)
	{
		gNotice.GCNoticeSendToAll(0,0,0,0,0,0,gMessage.GetMessage(543));
...
bool CCustomLukeHelper::NpcLukeHelper(LPOBJ lpNpc,LPOBJ lpObj) // OK
{
	...
	GCChatTargetSend(lpObj,lpNpc->Index,gMessage.GetMessage(544));

	gNotice.GCNoticeSendToAll(0,0,0,0,0,0,gMessage.GetMessage(545),lpObj->Name);
	...
	if(lpInfo != 0)
	{
		gItemBagManager.DropItemBySpecialValue(lpInfo->BagSpecial,lpObj,lpObj->Map,lpObj->X,lpObj->Y); 
	}

	return 1;
}

void CCustomLukeHelper::StartLukeHelper()
{
	...
	LogAdd(LOG_EVENT,"[Set LukeHelper Start] At %02d:%02d:00",hour,minute);
