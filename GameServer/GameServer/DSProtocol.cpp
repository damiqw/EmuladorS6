...
void DataServerProtocolCore(BYTE head,BYTE* lpMsg,int size) // OK
{
	...
	switch(head)
	{
...
#if(HAISLOTRING)
				case 0x01:
					g_pNewMyInventory.DGPetInventoryRecv((SDHP_PET_INVENTORY_RECV*)lpMsg);
					break;
#endif
			}
			break;
		...
		case 0xD3:
		{
			...
			case 0x40:
				gCustomRanking.RecvInfoCharTop((DATA_VIEWTOPRANKING*)lpMsg);
				break;
#endif
			}
			break;
...
void DGCharacterInfoRecv(SDHP_CHARACTER_INFO_RECV* lpMsg) // OK
{
	...
	gCustomBuyVip.GCBuyVipSend(lpObj->Index);
	...
	gPartyRestore.OnCharacterLogin(lpObj);
...
void DGGlobalPostRecv(SDHP_GLOBAL_POST_RECV* lpMsg) // OK
{
	...
	if(gServerInfo.m_CommandPostGlobalShowServerName == 1)
	{
		wsprintf(PostMessage,"[%s] %s",lpMsg->servername,lpMsg->message);
	}
	else
	{
		wsprintf(PostMessage,"%s",lpMsg->message);
	}

	switch(lpMsg->type)
	{
		case 0:
			PostMessage1(lpMsg->name,gMessage.GetMessage(69),PostMessage);
			break;
		case 1:
			PostMessage2(lpMsg->name,gMessage.GetMessage(69),PostMessage);
			break;
		case 2:
			PostMessage3(lpMsg->name,gMessage.GetMessage(69),PostMessage);
			break;
		case 3:
			PostMessage4(lpMsg->name,gMessage.GetMessage(69),PostMessage);
			break;
	}
}
...
void GDGlobalPostSend(int MapServerGroup,BYTE type,char* name,char* message,char* servername) // OK
{
	SDHP_GLOBAL_POST_SEND pMsg;
	...
	memcpy(pMsg.servername,servername,sizeof(pMsg.servername));
...
#if(FLAG)
	pMsg.Flag = lpObj->Flag;
	pMsg.CheckFlag = lpObj->CheckFlag;
#endif
...
void GDRankingCustomGreatPKSaveSend(int aIndex, DWORD number, DWORD score, DWORD rank) // OK
{
	LPOBJ lpObj = &gObj[aIndex];
...
//Set Coin System
void GDSetCoinSend(int aIndex, int value1, int value2, int value3, int value4, char * LogName) // OK
{
    LPOBJ lpObj = &gObj[aIndex];
    ...
    gDataServerConnection.DataSend((BYTE*)&pMsg,pMsg.header.size);
		...
		{
			lpObj->Ruud += value4;
			gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(726), value4);
		}
		...
		{
			lpObj->Ruud += value4;
			gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(725), value4);
		}
...
#if(NEW_POSTITEM)
void DGPostItemProc(BYTE* lpMsg, int size)
{
	...
	PMSG_POST_ITEM_SEND pMsg = { 0, };

	pMsg.header.set(0x78, sizeof(pMsg));
	//LogAdd(LOG_BLACK, "Send to CL %s", Data->chatid);//
	memcpy(pMsg.chatid, Data->chatid, sizeof(pMsg.chatid)-1);
	...
	short tmpservercode = Data->MapServerGroup;
	while (tmpservercode >= 20) { tmpservercode -= 20; }
	tmpservercode += 1;
	char szTemp[256];
	wsprintf(szTemp, "[Sub-%d] %s", tmpservercode, Data->pre);
