...
void EntryProc(HINSTANCE hins) // OK
{
	...
	if(GetLastError() == ERROR_ALREADY_EXISTS)
	{
		...
		wsprintf(path,"%s\\AntihackServer.ini",ConvertModuleFileName(name));

		GetPrivateProfileString("AntihackServerInfo","CustomerName","",CustomerName,sizeof(CustomerName),path);

		EncDecKey1 = GetPrivateProfileInt("AntihackServerInfo","EncDecKey1",0,path);

		EncDecKey2 = GetPrivateProfileInt("AntihackServerInfo","EncDecKey2",0,path);

		InternalVersion = GetPrivateProfileInt("AntihackServerInfo","InternalVersion",0,path);
