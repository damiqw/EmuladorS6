...
{
	this->ReadConfig("AntihackServerInfo",".\\AntihackServer.ini");
...
void CServerInfo::ReadStartupInfo(char* section,char* path) // OK
{
	...
	this->m_HackServerPort = GetPrivateProfileInt(section,"AntihackServerPort",0,path);
...
void CServerInfo::ReadConfig(char* section,char* path) // OK
{
	...
	this->m_WriteProtocolLog = GetPrivateProfileInt(section,"WriteProtocolLog",0,path);
