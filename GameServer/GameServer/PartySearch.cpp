	...
	if (lpMsg->SystemActive == true)
	{
		...
		if(it == this->m_Registered.end())
		{
			...
			if (info.RequirePassword)
				...
				gNotice.GCNoticeSendToAll(0,0,0,0,0,0,"New Auto Party created [%s] %s %d %d",gObj[aIndex].Name, gMapManager.GetMapName(gObj[aIndex].Map),gObj[aIndex].X,gObj[aIndex].Y);
		}
		else
		{
			it->second = info;
