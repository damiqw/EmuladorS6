...
static BYTE gObjInventoryInsertItemPos(int aIndex, CItem item, int pos, int RequestCheck)
{
...
static void gObjTempPShopItemBoxSet(BYTE * TempMap, int itempos, int xl, int yl, BYTE set_byte)
{
	int itemposx = (itempos - INVENTORY_EXT4_SIZE) % 8;
	int itemposy = (itempos - INVENTORY_EXT4_SIZE) / 8;
		...
		for(int botNum=0;botNum<MAX_BOTSTORE;botNum++)
		{
			if(this->bot[botNum].Enabled == true)
			{
				...
				if(result >= 0)
				{
					...
					//gObj[result].MuunInventory = new CItem[INVENTORY_SIZE];
					//gObj[result].MuunInventoryMap = new BYTE[INVENTORY_SIZE];
					...
					memset(&gObj[result].InventoryMap[0], (BYTE)-1, INVENTORY_SIZE);	//108+64+64
					memset(&gObj[result].InventoryMap1[0], (BYTE)-1, INVENTORY_SIZE);	//108+64+64
