...
void CExperienceTable::Load(char* path) // OK
{
	...
	try
	{
		while(true)
		{
			...
			info.ExperienceRate = (info.ExperienceRate == -1) ? 100 : info.ExperienceRate;

			info.MasterExperienceRate = lpMemScript->GetAsNumber();
			info.MasterExperienceRate = (info.MasterExperienceRate == -1) ? 100 : info.MasterExperienceRate;
...
int CExperienceTable::GetMasterExperienceRate(LPOBJ lpObj) // OK
{
	for(std::vector<EXPERIENCE_TABLE_INFO>::iterator it=this->m_ExperienceTableInfo.begin();it != this->m_ExperienceTableInfo.end();it++)
	{
		...
		return it->MasterExperienceRate;
	}
