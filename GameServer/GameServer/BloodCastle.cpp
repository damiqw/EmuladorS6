		...
		while(true)
		{
			...
			while(true)
			{
				...
				else if(section == 5)
				{
					...
					this->m_BloodCastleRewardValue[level][3] = lpMemScript->GetAsNumber();
				}
				else
				{
					break;
...
void CBloodCastle::SetState_START(BLOOD_CASTLE_LEVEL* lpLevel) // OK
{
	...
	lpLevel->TimeCount = 0;
	lpLevel->CurMonster = 0;
	lpLevel->MaxMonster = this->GetUserCount(lpLevel)*240; //monsters aca se aumenta la primera entrada
	...
	for(int n=0;n < MAX_BC_USER;n++)
	{
		if(OBJECT_RANGE(lpLevel->User[n].Index) != 0)
		{
			if(lpLevel->User[n].Index == lpLevel->WinnerIndex || (OBJECT_RANGE(lpLevel->WinnerIndex) != 0 && gParty.IsMember(gObj[lpLevel->WinnerIndex].PartyNumber,lpLevel->User[n].Index) != 0))
			{
				...
				gShopPointEx.AddEventBonus(lpLevel->User[n].Index, ShopPointExEvent::BC);

				GDRankingBloodCastleSaveSend(lpLevel->User[n].Index,lpLevel->User[n].Score);

				gQuestWorldObjective.QuestWorldObjectiveSpecial(&gObj[lpLevel->User[n].Index], QUEST_WORLD_OBJECTIVE_BLOOD_CASTLE_CLEAR, lpLevel->Level + 1);
			}
			else
			{
				this->GiveUserRewardExperience(lpLevel,lpLevel->User[n].Index);
...
void CBloodCastle::SetStage2(BLOOD_CASTLE_LEVEL* lpLevel) // OK
{
	...
	lpLevel->MaxMonster = this->GetUserCount(lpLevel)*100; //segunda entrada despues de romper el porton 
...
void CBloodCastle::GiveUserRewardItem(BLOOD_CASTLE_LEVEL* lpLevel,int aIndex) // OK
{
	...
	if (this->m_BloodCastleRewardValue[lpLevel->Level][0] > 0 || this->m_BloodCastleRewardValue[lpLevel->Level][1] > 0 || this->m_BloodCastleRewardValue[lpLevel->Level][2] > 0 || this->m_BloodCastleRewardValue[lpLevel->Level][3] > 0)
	{
		//Reward
		GDSetCoinSend(lpUser->Index, this->m_BloodCastleRewardValue[lpLevel->Level][0], this->m_BloodCastleRewardValue[lpLevel->Level][1], this->m_BloodCastleRewardValue[lpLevel->Level][2], this->m_BloodCastleRewardValue[lpLevel->Level][3],"BC");
...
void CBloodCastle::MonsterDieProc(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	if(lpObj->Class == 131 && lpObj->Index == this->m_BloodCastleLevel[level].GateIndex && this->m_BloodCastleLevel[level].Stage == 2)
	{
		...
		gQuestWorldObjective.QuestWorldObjectiveSpecial(lpTarget, QUEST_WORLD_OBJECTIVE_BLOOD_CASTLE_GATE_DESTROY, level + 1);
		return;
	}
