...
void CObjectManager::CharacterGameCloseSet(int aIndex,int type) // OK
{
	...
	if(lpObj->Connected == OBJECT_ONLINE)
	{
		...
		if(lpObj->DieRegen != 0)
		{
			gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(641));
			return;
		}
	}
...
bool CObjectManager::CharacterGetRespawnLocation(LPOBJ lpObj) // OK
{
	...
	if(gMapManager.GetRespawnGate(lpObj->Map) != -1)
	{
		result = gGate.GetGate(gMapManager.GetRespawnGate(lpObj->Map),&gate,&map,&x,&y,&dir,&level);
	}
...
void CObjectManager::CharacterCalcExperienceAlone(LPOBJ lpObj,LPOBJ lpMonster,int damage,int flag,bool type,int AttackDamage) // OK
{
	...
	DWORD experienceToAdd = (experience > 0x7FFFFFFF) ? 0x7FFFFFFF : (DWORD)experience;

	if(this->CharacterLevelUp(lpObj,experienceToAdd,gServerInfo.m_MaxLevelUp,EXPERIENCE_COMMON) == 0)
	{
		if (gServerInfo.m_DisableExpMessage == 0) 
		{ 
			GCMonsterDieSend(lpObj->Index,lpMonster->Index,experienceToAdd,((type==0)?AttackDamage:0),flag);
		}
	}
	else
	{
...
void CObjectManager::CharacterCalcExperienceParty(LPOBJ lpObj,LPOBJ lpMonster,int damage,int flag) // OK
{
	...
	for(int n=0;n < MAX_PARTY_USER;n++)
	{
		...
		if(this->CharacterLevelUp(lpTarget,experienceToAdd,gServerInfo.m_MaxLevelUp,EXPERIENCE_PARTY) == 0)
		{
			if (gServerInfo.m_DisableExpMessage == 0) 
			{ 
				GCMonsterDieSend(lpTarget->Index,lpMonster->Index,experienceToAdd,damage,flag);
			}
		}
		else
		{
...
void CObjectManager::CharacterMakePreviewCharSet(int aIndex) // OK
{
	...
	for (int n = 0; n < INVENTORY_WEAR_SIZE; n++)
	{
		if (n == 0 || n == 1)
		{
			...
			{
				TempInventory[n] = lpObj->EquipInventory[n].m_Index;
			}
			else if (lpObj->Inventory[n].IsItem() != 0)
			{
		...
		else
		{
			...
			if (n < 8)
			{
				if (lpObj->EquipInventory[n].IsItem() != 0)
				{
					TempInventory[n] = lpObj->EquipInventory[n].m_Index % MAX_ITEM_TYPE;
				}
				else if (lpObj->Inventory[n].IsItem() != 0)
				{
	...
	for (int n = 0; n < 7; n++)
	{
		if (TempInventory[n] != 0x1FF && TempInventory[n] != 0xFFFF)
		{
			...
			{
				level |= ((lpObj->EquipInventory[n].m_Level - 1) / 2) << (n * 3);
				lpObj->CharSet[10] |= ((lpObj->EquipInventory[n].m_NewOption & 0x3F) ? 2 : 0) << table[n];
				lpObj->CharSet[11] |= ((lpObj->EquipInventory[n].m_SetOption & 0x03) ? 2 : 0) << table[n];
			}
#else
			level |= ((lpObj->Inventory[n].m_Level - 1) / 2) << (n * 3);
...
bool CObjectManager::CharacterUseJewelOfBles(LPOBJ lpObj,int SourceSlot,int TargetSlot) // OK
{
	...
	if (gCustomJewelDisable.CheckCustomJewelDisableByItem(lpItem->m_Index) && gCustomJewelDisable.JewelOfBless == 1)
	{
		return 0;
	}
...
bool CObjectManager::CharacterUseJewelOfSoul(LPOBJ lpObj,int SourceSlot,int TargetSlot) // OK
{
	...
	if (gCustomJewelDisable.CheckCustomJewelDisableByItem(lpItem->m_Index) && gCustomJewelDisable.JewelOfSoul == 1)
	{
		return 0;
	}
...
bool CObjectManager::CharacterUseJewelOfLife(LPOBJ lpObj,int SourceSlot,int TargetSlot) // OK
{
	...
	if (gCustomJewelDisable.CheckCustomJewelDisableByItem(lpItem->m_Index) && gCustomJewelDisable.JewelOfLife == 1)
	{
		return 0;
	}
	...
	for(int n=10;n < 12;n++)
	{
		if(lpObj->Inventory[n].IsItem() != 0)
		{
			...
			if(result != 0)
			{
				...
				if (gInvoker.InvokerSummon(lpObj->Index) != 0)
				{
					gObjViewportListProtocolCreate(lpObj);
					this->CharacterUpdateMapEffect(lpObj);
				}
...
bool CObjectManager::CharacterInfoSet(BYTE* aRecv,int aIndex) // OK
{
	...
	if(gMapManager.GetSpawnGate(lpObj->Map) != -1)
	{
		result = gGate.GetGate(gMapManager.GetSpawnGate(lpObj->Map),&gate,&map,&x,&y,&dir,&level);
	}
	...
	lpObj->CheckFlag = lpMsg->CheckFlag;
#endif
	memcpy(lpObj->ServerName,lpMsg->ServerName,sizeof(lpObj->ServerName));
...
void CObjectManager::CharacterLifeCheck(LPOBJ lpObj,LPOBJ lpTarget,int damage,int DamageType,int flag,int type,int skill,int ShieldDamage) // OK
{
	...
	if(lpTarget->Life <= 0 && lpTarget->Live != 0)
	{
		...
		if(lpTarget->Type == OBJECT_MONSTER)
		{
			...
			gCustomBuffMonster.AddMonsterBonus(lpTarget,&gObj[SummonIndex]);

			gShopPointEx.AddMonsterBonus(lpTarget,&gObj[SummonIndex]);

			gCustomDropCoin.AddMonsterBonus(lpTarget, &gObj[SummonIndex]);
